...
from .serializers import PointMessageSerializer, PointSearchSerializer, PointSerializer
    ...
    def search(self, request):
        ...
        points = (
            Point.objects.filter(
                location__distance_lte=(location, D(m=radius)), created_by=request.user  # Фильтр по пользователю
            )
...
class PointMessageViewSet(viewsets.ModelViewSet):
    ...
    def search(self, request):
        ...
        points_in_radius_ids = Point.objects.filter(location__distance_lte=(location, D(m=radius))).values_list(
            "id", flat=True
        )

        # получаем сообщения в найледенных точках
        messages = (
            PointMessage.objects.filter(point_id__in=points_in_radius_ids)
